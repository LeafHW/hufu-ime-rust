...
//!
//! 【Linux 适配 2026-09-19】原生打分器只有 Windows 版
//! `TigerClaw.Sentence.Native.dll`（且它自身是 MSVC 工程），非 Windows
//! 平台提供空实现：`try_new` 返回 None → 调用方自动回落纯 Rust 重排
//! （host.rs 的双引擎逻辑里「native 缺失即用 Rust」本就是默认路径）。
...
mod imp {
    ...
    unsafe fn sym<T>(h: windows_sys::Win32::Foundation::HMODULE, name: &str) -> Option<T> {
        let n = cstr(name);
...
#[cfg(windows)]
pub use imp::NativeScorer;

/// 非 Windows：无原生 dll 资产，恒回落纯 Rust 重排。
#[cfg(not(windows))]
pub struct NativeScorer;
...
impl NativeScorer {
    pub fn try_new(
        _extra_dirs: &[&std::path::Path],
        _model: &std::path::Path,
    ) -> Option<NativeScorer> {
        None
    }

    /// 占位：非 Windows 下 `try_new` 恒 None，永不构造；仅为调用点
    /// （host.rs 双引擎分支）类型检查保留同签名。
    pub fn score(&self, _ctx: &str, cands: &[String]) -> Vec<f64> {
        vec![f64::NEG_INFINITY; cands.len()]
    }
}
