...
fn handle_set_focus(
    ...
    // 判定就位后成为冗余，且误伤真实切格：顶屏上屏后立刻 Tab/回车切
    ...
    if composing && !preedit.is_empty() {
        ...
        //（整句/成词显示文本）；编码字母形态（preedit==raw，顶屏
    ...
    fn do_edit_session(&self, ec: u32) -> Result<()> {
        ...
        match &self.op {
            ...
            Op::CommitAndRepreedit(commit_text, preedit) => {
                ...
                g.composition = None;
                // 2) 重开组段显示剩余预编辑
                // 【二十四次修正】顶屏上屏「字宽<编码宽」光标回退——置
                // 顶屏钳位许可：候选窗原地等光标追回再跟（点击换位的
                // SP 路径不置，不受影响）。
                if let Some(c) = g.cand2.as_mut() {
                    c.forward_hold = true;
                }
...
fn hupo_qie_step(g: &mut Shared, ctx: &ITfContext, ec: u32) {
    ...
    // 会把整句流顶屏上屏误判为段结束：轻推链冻结+raw==1 立段跳回
    ...
    if g.cur_raw_len == 1 {
        // 【五十九修·句内顶屏保护】顶屏上屏后剩余 raw==1 不是新句
