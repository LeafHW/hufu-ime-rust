...
### 虎码打字规则：一简字只单独打，句子里用二简以上（用户钦定）

- **一简**（`u`=的、`t`=我 等 26 个单码字）**只在单独打一个字时使用**：`u`+空格 → 的。
- **句子里（多码整句）一简永远无效**，必须用该字的二简及以上编码：
  「你的」= `jxue`（不是 `jxu`）；「的话」= `uesa`（不是 `usa`）。
- 这条规则由整句解码器的**段跨≥2 规则**强制执行（`hufu-sentence` lib.rs，
  与 tiger_sentence.lua L562 同款）：多码 raw 中跨度 1 的段被禁止，故
  一简编码进不了句子格子。虎爪/Rime 虎整句同此规则——两边行为一致。
- 例外通道：若词条本身在码表里有多字词编码（如 `eyiahx 窒闷`），
  词段跨度≥2 自然合法，可与任何前段组句。
